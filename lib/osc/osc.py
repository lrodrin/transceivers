...
class OSC():
	def __init__(self, trx_mode, rx_ID):
		...
		self.rx_ID=rx_ID   #To identify the channel of the DAC to be used and the local files to use for storing data. Tx_id when Mode 0 (METRO_1 scenario) is equivalent to select the Openconfig client. Tx_id when Mode 1 (METRO_2 scenario) is equivalent to select the S_BVT, which includes 2 clients multiplexed in a single optical channel. Due to hardware limitations in this last case (METRO2 scenario) tx_ID will be always 0.	
