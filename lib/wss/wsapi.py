...
def ws_create_waveshaper2(name, wsconfig, serial, spi):
    if wsapi is None:
        return -1
    p = create_string_buffer(name)
    ...
    return wsapi.ws_create_waveshaper2(p, wsconfig, serial, spi, 0), p.raw.strip("\x00")
...
def ws_create_waveshaper3(name, wsconfig, serial, spi, loadflag):
    if wsapi is None:
        return -1
    p = create_string_buffer(name)
    ...
    return wsapi.ws_create_waveshaper3(p, wsconfig, serial, spi, loadflag), p.raw.strip("\x00")
...
def ws_load_predefinedprofile(name, filtertype, center, bandwidth, attn, port):
    ...
    return wsapi.ws_load_predefinedprofile(name, filtertype, c_float.from_param(center), c_float.from_param(bandwidth),
                                           c_float.from_param(attn), port)
